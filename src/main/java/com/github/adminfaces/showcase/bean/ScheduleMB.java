...
public class ScheduleMB implements Serializable {
    ...
    public void init() {
        ...
        eventModel.addEvent(createEvent("Champions League Match", previousDay8Pm(), previousDay11Pm()));
        eventModel.addEvent(createEvent("Birthday Party", today1Pm(), today6Pm()));
        eventModel.addEvent(createEvent("Breakfast at Tiffanys", nextDay9Am(), nextDay11Am()));
        eventModel.addEvent(createEvent("Plant the new garden stuff", theDayAfter3Pm(), fourDaysLater3pm()));
        ...
        lazyEventModel = new LazyScheduleModel() {
            ...
            public void loadEvents(LocalDateTime start, LocalDateTime end) {
                ...
                addEvent(createEvent("Lazy Event 1", random, random));

                random = getRandomDate(end);
                addEvent(createEvent("Lazy Event 2", random, random));
            }
        };
    }

    private ScheduleEvent<?> createEvent(String description, LocalDateTime start, LocalDateTime end) {
        return DefaultScheduleEvent.builder().description(description).startDate(start).endDate(end).build();
    }
    ...
    public void onDateSelect(SelectEvent selectEvent) {
        event = createEvent("", (LocalDateTime) selectEvent.getObject(), (LocalDateTime) selectEvent.getObject());
    }
